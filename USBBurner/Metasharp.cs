...
namespace Metasharp
{
    public class DriveManager
    {
        ...
        /// <returns>true if success, false if failure</returns>
        public static bool FormatDrive(Process prc, char driveLetter, string label = "", string fileSystem = "NTFS", bool quickFormat = true, bool enableCompression = false, int? clusterSize = null)
        {
            return FormatDrive_CommandLine(prc,driveLetter, label, fileSystem, quickFormat, enableCompression, clusterSize);
        }
        ...
        /// <param name="clusterSize">cluster size (default=null for auto). Possible value depends on the file system : 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, ...</param>
        /// <returns>true if success, false if failure</returns>
        public static bool FormatDrive_CommandLine(Process formatProcess, char driveLetter, string label = "", string fileSystem = "NTFS", bool quickFormat = true, bool enableCompression = false, int? clusterSize = null)
        {
        ...
        public static bool RenameDrive(Process formatProcess, char driveLetter, string label = "")
        {
            ...
            try
            {
                var di = new DriveInfo(drive);
                var psi = new ProcessStartInfo();
                psi.FileName = "label.exe";
                psi.WorkingDirectory = Environment.SystemDirectory;
                psi.Arguments = drive+" "+label;
                psi.UseShellExecute = false;
                psi.CreateNoWindow = true;
                psi.RedirectStandardOutput = true;
                psi.RedirectStandardInput = true;
